...
class DbManager extends BaseManager
{
     ...
     */
    public $itemTable = 'module_rbac_auth_item';
    /**
     * @var string the name of the table storing authorization item hierarchy. Defaults to "auth_item_child".
     */
    public $itemChildTable = 'module_rbac_auth_item_child';
    /**
     * @var string the name of the table storing authorization item assignments. Defaults to "auth_assignment".
     */
    public $assignmentTable = 'module_rbac_auth_assignment';
    /**
     * @var string the name of the table storing rules. Defaults to "auth_rule".
     */
    public $ruleTable = 'module_rbac_auth_rule';
    /**
     * @var CacheInterface|array|string the cache used to improve RBAC performance. This can be one of the
     *      following:
     *
    ...
    protected function checkAccessFromCache($user, $itemName, $params, $assignments)
    {
        ...
        Yii::debug(
            $item instanceof Role ? "Checking role: $itemName" : "Checking permission: $itemName",
            __METHOD__
        );
    ...
    protected function getItem($name)
    {
        ...
        if (!empty($this->items) && !empty($this->items[$name])) {
    ...
    protected function populateItem($row)
    {
        ...
        if (!isset($row['data']) || ($data = @unserialize(
        ...
        return new $class(
            [
                ...
                'module_en' => $row['module_en'],
                'module_fa' => $row['module_fa'],
            ]
        );
    }
     ...
     * @param string|int $userId the user ID (see [[\yii\web\User::id]])
     *
     * @return Permission[] all inherited permissions that the user has. The array is indexed by the permission
     *                      names.
     * @since 2.0.7
     */
    ...
    public function revoke($role, $userId)
    {
        ...
        return $this->db->createCommand()
                ->delete(
                    $this->assignmentTable,
                    ['user_id' => (string)$userId, 'item_name' => $role->name]
                )
                ->execute() > 0;
    }
