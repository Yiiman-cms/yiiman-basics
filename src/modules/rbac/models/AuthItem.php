<?php
	...
	abstract class AuthItem extends Model {
		...
		/**
		 * @param \YiiMan\YiiBasics\modules\rbac\models\Item $item
		...
		public function __construct( $item , $config = array() ) {
			...
			if ( $item !== null ) {
				...
				$this->module_en    = $item['module_en'];
				$this->type=   !empty($item['type'])? $item['type']:\YiiMan\YiiBasics\modules\rbac\models\Item::TYPE_PERMISSION;
				$this->module_fa    = $item['module_fa'];
				$this->data        = $item['data'] === null ? null : Json::encode( $item['data'] );
			}
			parent::__construct( $config );
		}
		
		public function uniquee() {
		...
		public function rules() {
			return [
				...
				[
					...
					'uniquee' ,
					'when' => function () {
						return $this->isNewRecord || ( !empty($item) && $this->item->name != $this->name );
					}
				] ,
