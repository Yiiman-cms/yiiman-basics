...
//    }
    ...
    private function loadData($model, $id)
    {
        ...
        // < Load Fields >
        {
            $fields = PosttypesFields::find()->select([
                'fieldName',
                'content'
            ])->where(['posttype' => $model->id])->all();
            ...
            // < Load relations >
            {
                $relations = PosttypesFk::find()->select([
                    'posttype_to',
                    'posttype_type_to'
                ])->where(['posttype_from' => $model->id])->asArray()->all();
                if (!empty($relations)) {
                    ...
                    try {
                        ...
                        switch ($config['type']) {
                            ...
                            case Posttypes::INPUT_RADIO:
                                if (!empty($field)) {
                                    foreach ($field as $f) {
                                        $fieldModel = PosttypesFields::findOne([
                                            'fieldName' => $name,
                                            'posttype'  => $model->id,
                                            'content'   => $f
                                        ]);
...
//                            $relatedModel = PosttypesFields::findOne(['fieldName' => $name, 'posttype' => $model->id]);
                            ...
                            case Posttypes::INPUT_MULTIPLE:
                                ...
                                if (!empty($field)) {
                                    $fieldModel2 = PosttypesMultiple::find()->where([
                                        'posttype_id' => $model->id,
                                        'fieldName'   => $name
                                    ])->all();
                        ...
                        switch ($field['type']) {
                            ...
                            case Posttypes::INPUT_RADIO:
                                if (empty($model->fields[$name])) {
                                    $savedOnDb = PosttypesFields::find()->where([
                                        'posttype'  => $model->id,
                                        'fieldName' => $name
                                    ])->all();
                                    if (!empty($savedOnDb)) {
