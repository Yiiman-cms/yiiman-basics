...
class Posttypes extends \YiiMan\YiiBasics\lib\ActiveRecord
{
     ...
     */
    public
    static function getPostTypeWithCheckedFields(
        $posttype,
        $fieldName,
        $fieldContent = '',
        $justActived = false
        ...
        if (!(is_int($posttype) || is_integer($posttype) || is_array($posttype))) {
            if ($justActived) {
                ...
                $postConvertedIds = Posttypes::find()->select(['id'])->where([
                    'postType' => $posttype,
                    'status'   => Posttypes::STATUS_ACTIVE
                ])->asArray()->all();
            }
        ...
        if (empty($fieldContent)) {
            $postModels = \YiiMan\YiiBasics\modules\posttypes\models\PosttypesFields::find()->select(['posttype'])->where([
                'fieldName' => $fieldName,
                'posttype'  => $postConvertedIds
            ])->asArray()->all();
        } else {
            $postModels = \YiiMan\YiiBasics\modules\posttypes\models\PosttypesFields::find()->select(['posttype'])->where([
                'fieldName' => $fieldName,
     ...
     */
    public
    static function getMappedPosts(
        $posttype,
        $attr1 = 'id',
        $attr2 = 'title'
    ) {
     ...
     */
    public
    static function getActivePosts(
        $posttype = '',
        $limit = null,
        $ids = []
    ) {
     ...
     */
    public
    static function get_posts_has_related_target_data(
        $posttype
    ) {
    ...
    static function render(
        ...
        switch ($type) {
            ...
            case Posttypes::INPUT_IMAGE_SINGLE:
                ...
                $out .= $model->image_input_widget($form, $column['label'], true, ['image'], [], 'fields['.$name.']', 1,
                    false);
                $out .= '</div>';
                break;
            ...
            case Posttypes::INPUT_RELATION_SINGLE:
                ...
                $out .= $form
                    ...
                    ->widget(Select2::className(),
                        [
                            'data'    =>
                                !empty($model::getPosts($name)) ? ArrayHelper::map($model::getPosts($name), 'id',
                                    'title') : [],
                            'options' =>
                                [
                                    'dir'         => \YiiMan\YiiBasics\lib\i18n\Layout::run(),
    ...
    public function getPosttypesFields($force = false)
    {
        if (
            ...
            if (!empty($data)) {
                ...
                self::$data_loaded[$this->id]['fields'] = ArrayHelper::index(self::$data_loaded[$this->id]['fields'],
                    null, 'fieldName');
            }
        }
    ...
    public function getPosttypesRelations($force = false)
    {
        // < load Relations >
        {
            ...
            if (!empty(self::$data_loaded[$this->id]['relations']) && $force == false) {
                ...
                $relations = PosttypesFk::find()->select([
                    'posttype_to',
                    'posttype_type_to'
                ])->where(['posttype_from' => $this->id])->asArray()->all();
                if (!empty($relations)) {
                    ...
                    foreach ($fieldModel as $item) {
                        ...
                        switch ($item->type) {
                            ...
                            case MultipleInputColumn::TYPE_DROPDOWN:
                                if (!empty($configs[$posttype][$item->fieldName]['fields'][$item->key]['toModel'])) {
                                    ...
                                    if ($model == 'YiiMan\YiiBasics\modules\posttypes\models\Posttypes') {
                                        $outItem[$item->fieldName][$item->index][$item->key] = self::getPost('',
                                            $item->value);
                                    } else {
                            ...
                            case MultipleInputColumn::TYPE_RADIO_LIST:
                                ...
                                $fieldModelFields = PosttypesMultipleFields::find()->select([
                                    'key',
                                    'value'
                                ])->where(['multiple_field_id' => $item->id])->asArray()->all();
...
//                    }
                ...
                if ($modelNamePos && $modelNamePos == ($len - 1)) {
                    ...
                    if (!empty(self::$data_loaded[$this->id]['fields'][$name])) {
                        ...
                        switch ($config['type']) {
                            case Posttypes::INPUT_CHECKBOX:
                            case Posttypes::INPUT_MULTI_SELECT:
                                $ids = ArrayHelper::getColumn(self::$data_loaded[$this->id]['fields'][$name],
                                    'content');
    ...
    function getRelatedModel(
        ...
        $out = PosttypesFk::findOne([
            'posttype_type_from' => $this->postType,
            'posttype_type_to'   => $relationName_to,
            'posttype_from'      => $this->id
        ]);
        if (!empty($out) && $justFK) {
            return $out;
        }
    ...
    function getRelatedModels(
        ...
        if ($invert) {
            $out = PosttypesFk::find()->where([
                'posttype_type_to'   => $this->postType,
                'posttype_type_from' => $relationName_to,
                'posttype_to'        => $this->id
            ]);
        } else {
            $out = PosttypesFk::find()->where([
