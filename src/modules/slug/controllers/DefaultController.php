...
SQL;
				...
				if ( ! empty( $model ) ) {
				    ...
				    if ($error){
                        return [ 'status' => 'error' , 'message' => '<span style="color: red">این نامک قبلا رزرو شده است</span>' ];
                    }else{
                        return [ 'status' => 'success' , 'message' => '<span style="color: green">این نامک مورد تایید است</span>' ];
                    }
                } else {
                    return [ 'status' => 'success' , 'message' => '<span style="color: green">این نامک مورد تایید است</span>' ];
				}
...
SQL;
				...
				if (  empty( $model ) ) {
					if ($relatedModel->isNewRecord){
						...
						$model=Slug::findOne( ['table_id'=>$relatedModel->id,'table_name'=>$relatedModel::tableName()]);
						if (!empty( $model)){
							$model->slug=$post['Slug']['slug'];
							$model->save();
						}else{
                ...
                if (empty($post['slug'])){
                    $slugModel=Slug::findOne(['table_id'=>$relatedModel->id,'table_name'=>$relatedModel::tableName()]);
                    if (!empty($slugModel)){
                        $slugModel->delete();
                    }
                }
