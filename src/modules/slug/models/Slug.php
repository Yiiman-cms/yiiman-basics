...
class Slug extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['slug', 'table_name', 'table_id'], 'required'],
            [['table_id'], 'integer'],
            [['slug', 'table_name'], 'string', 'max' => 255],
        ];
    }
    ...
    {
        $model = self::findOne(['table_name' => $model::tableName(), 'table_id' => $model->id]);
        if (!empty($model)) {
            return $model->slug;
        } else {
            return '';
    ...
    {
        $model = self::findOne(['table_name' => $table, 'table_id' => $id]);
        if (!empty($model)) {
            return $model->slug;
        } else {
            return '';
