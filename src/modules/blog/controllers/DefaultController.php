	...
	class DefaultController extends \YiiMan\YiiBasics\lib\Controller {
		...
		public function actionDelete( $id ) {
			...
			if (!empty( $tags)){
				if (is_array( $tags)){
					foreach ($tags as $tag){
						$count=Metadata::find()->where(['module_id'=>$model->id,'meta_key'=>'ARTICLE_SEO_TAG','content'=>$tag->content])->count();
						if ($count==1){
							Metadata::find()->where(['module_id'=>$model->id,'meta_key'=>'ARTICLE_SEO_TAG','content'=>$tag->content])->one()->delete();
						}
					}
				}else{
					$count=Metadata::find()->where(['module_id'=>$model->id,'meta_key'=>'ARTICLE_SEO_TAG','content'=>$tags->content])->count();
					if ($count==1){
						Metadata::find()->where(['module_id'=>$model->id,'meta_key'=>'ARTICLE_SEO_TAG','content'=>$tags->content])->one()->delete();
					}
				}
