...
class Menu extends ActiveRecord
{
    ...
    public function rules()
    {
        return
            [
                [['location', 'status', 'related_id', 'index', 'language', 'parent_id'], 'integer'],
                [['title', 'url', 'icon', 'image', 'type'], 'string', 'max' => 255],
            ];
    }
    ...
    public static function getAllActived($location, $array = true)
    {
        if (empty(self::$menus)) {
            ...
            $query = self::find()
                ->where(['status' => self::STATUS_PUBLISHED, 'location' => $location, 'language' => Yii::$app->Language->currentID()])
            ...
            if ($array && !empty(self::$menus[$location]) && is_array(self::$menus[$location])) {
                return self::$menus[$location];
            } else if (!$array && !empty(self::$menus[$location]) && is_object(self::$menus[$location])) {
                return self::$menus[$location];
            }
        }
    }
    ...
    public static function getAllActiveForWidget($location)
    {
        ...
        $all[] = ['label' => \Yii::t('site', 'صفحه اصلی'), 'url' => $homeUrl];
                ...
                // < calculate URL >
                {
                    ...
                    if (is_string($items[$id]['url']) && strlen($items[$id]['url']) < 2000) {
                        ...
                        if (strpos($patternSche, '{schemes}') !== false) {
                            $pattern = str_replace('{schemes}', '(' . implode('|', ['http', 'https']) . ')', $patternSche);
                        } else {
                            $pattern = $patternSche;
                        }
    ...
    {
        $slug = Slug::findOne(['table_name' => self::tableName(), 'table_id' => $model['id']]);
        if (!empty($slug)) {
            return $slug->slug;
        } else {
            return '';
