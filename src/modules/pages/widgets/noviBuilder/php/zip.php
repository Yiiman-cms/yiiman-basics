...
class Zip
{
    ...
    public function addFile($data, $filePath, $timestamp = 0, $fileComment = null, $compress = true)
    {
        ...
        if ($compress) {
            ...
            $gzData = substr(substr($gzTmp, 0, strlen($gzTmp) - 4),
                2); // gzcompress adds a 2 byte header and 4 byte CRC we can't use.
        ...
        $this->buildZipEntry($filePath, $fileComment, $gpFlags, $gzType, $timestamp, $fileCRC32, $gzLength, $dataLength,
    ...
    private function buildZipEntry(
        ...
        if (extension_loaded('mbstring')) {
            ...
            $isCommentUTF8 = !empty($fileComment) && mb_check_encoding($fileComment,
                    "UTF-8") && !mb_check_encoding($fileComment, "ASCII");
    ...
    public function addDirectory($directoryPath, $timestamp = 0, $fileComment = null)
    {
        ...
        if (strlen($directoryPath) > 0) {
            $this->buildZipEntry($directoryPath.'/', $fileComment, "\x00\x00", "\x00\x00", $timestamp,
                "\x00\x00\x00\x00", 0, 0, self::EXT_FILE_ATTR_DIR);
            return true;
        }
        return false;
    }
    ...
    function sendZip($fileName, $contentType = "application/zip")
    {
        ...
        $headerFile = null;
        $headerLine = null;
        if (!headers_sent($headerFile,
                $headerLine) or die("<p><strong>Error:</strong> Unable to send file $fileName. HTML Headers have already been sent from <strong>$headerFile</strong> in line <strong>$headerLine</strong></p>")) {
