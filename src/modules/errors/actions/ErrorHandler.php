 ...
 */
	...
	namespace YiiMan\YiiBasics\modules\errors\actions;
	...
	class ErrorHandler extends Action {
		...
		protected function getExceptionName() {
			...
			if ( $code = $this->getExceptionCode() ) {
				$pname=$this->persianCodes( $code);
				if (!empty($pname)){
					$name=$pname;
				}
				
				$name .= " ($code)";
			}elseif($code=$this->exception->statusCode){
				$pname=$this->persianCodes( $code);
				if (!empty($pname)){
					$name=$pname;
				}
		...
		public function persianCodes($code){
			switch ( $code ) {
				case 100:
					$name = 'ادامه';
					break;
				case 101:
					$name = 'در حال تعویض پروتکول ها';
					break;
				case 102:
					$name = 'در حال پردازش درخواست شما';
					break;
				case 200:
					$name = 'موفقیت آمیز';
					break;
				case 203:
					$name = 'اطلاعات نامعتبر';
					break;
				case 204:
					$name = 'بدون محتویات';
					break;
				...
				case 403:
					...
					break;
				case 404:
					$name = 'ظاهرا گم شده اید';
					break;
				case 405:
					$name='روش درخواست، اشتباه است';
					break;
				case 500:
					$name='خطای داخلی سرور';
					break;
