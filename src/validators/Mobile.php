<?php
	...
	use YiiMan\YiiBasics\lib\phoneNumber\NumberParseException;
	use YiiMan\YiiBasics\lib\phoneNumber\PhoneNumberFormat;
	use YiiMan\YiiBasics\lib\phoneNumber\PhoneNumberUtil;
	...
	class Mobile extends Validator
	{
		...
		public function validateAttribute($model, $attribute)
		{
			...
			if (!isset($country) && $this->strict) {
				$this->addError($model, $attribute, \Yii::t('validator', 'For phone validation country required'));
				return false;
			}
			...
			try {
				...
				if ($phoneUtil->isValidNumber($numberProto)) {
					if (is_numeric($this->format)) {
						$model->$attribute = $phoneUtil->format($numberProto, PhoneNumberFormat::E164);
					}
					return true;
				} else {
					$this->addError($model, $attribute, \Yii::t('validator', 'Phone number does not seem to be a valid phone number'));
					return false;
				}
			} catch (NumberParseException $e) {
				
				$this->addError($model, $attribute, \Yii::t('validator', 'Unexpected Phone Number Format'));
			} catch (Exception $e) {
				$this->addError($model, $attribute, \Yii::t('validator', 'Unexpected Phone Number Format or Country Code'));
			}
		}
