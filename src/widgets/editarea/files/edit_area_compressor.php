...
class Compressor
{
    ...
    function check_gzip_use()
    {
        ...
        if (!$desactivate_gzip && $this->param['use_gzip'] && (in_array('gzip', $encodings) || in_array('x-gzip',
                    $encodings) || isset($_SERVER['---------------'])) && function_exists('ob_gzhandler') && !ini_get('zlib.output_compression')) {
    ...
    function check_cache()
    {
        ...
        if (file_exists($this->cache_file)) {
            ...
            if (filemtime($this->cache_file) >= $cache_date) {
                ...
                if (isset($_SERVER["HTTP_IF_MODIFIED_SINCE"]) && strcasecmp($_SERVER["HTTP_IF_MODIFIED_SINCE"],
                        $last_modified) === 0) {
    ...
    function send_datas()
    {
        if ($this->param['debug']) {
            ...
            $header .= sprintf("javascript size: based files: %s => PHP COMPRESSION => %s ", $this->file_loaded_size,
                strlen($this->datas));
            if ($this->use_gzip) {
