...
class MessageSource extends Component
{
    ...
    protected function translateMessage($category, $message, $language)
    {
        ...
        $cache = $this->loadCache($category, $message, $language);
        ...
        if (isset($this->_messages[$key][$message]) && $this->_messages[$key][$message] !== '') {
            $this->saveCacheMessage($category, $message, $this->_messages[$key][$message], $language);
                    ...
                    if (!file_exists($tmp.'/translates.php')) {
                        ...
                        // < Load Cache >
                        {
                            ...
                            if (empty($tmp)) {
                                ...
                                self::$cache = $tmp;
                                if (!empty(self::$cache[$category][$message])) {
                                    return self::$cache[$category][$message];
                                } else {
                                    return false;
                                }
                            }
    ...
    public function saveCacheMessage($category, $message, $translate, $language)
    {
        ...
        if (!file_exists($tmp.'/translates.php')) {
            ...
            if (is_int(self::$cache)) {
                self::$cache = [];
            }

            self::$cache[$category][$message] = $translate;
            $file = fopen($tmp.'/translates.php', 'w+');
            fwrite($file, $this->generatePHPFile_array(self::$cache));
            fclose($file);
        }
    }
