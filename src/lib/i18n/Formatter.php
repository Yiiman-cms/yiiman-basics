...
class Formatter extends Component
{
    ...
    public function init()
    {
        ...
        if ($this->booleanFormat === null) {
            $this->booleanFormat = [
                Yii::t('yii', 'No', [], $this->language),
                Yii::t('yii', 'Yes', [], $this->language)
            ];
        }
    ...
    protected function asDecimalStringFallback($value, $decimals = 2)
    {
            ...
            if ($fractionalPart === null) {
                ...
                if ((int) substr($fractionalPart, $cursor, 1) >= 5) {
                    while (--$cursor >= 0) {
                        ...
                        $fractionalPart = substr($fractionalPart, 0, $cursor).$oneUp.substr($fractionalPart,
                                $cursor + 1);
        ...
        if ($carry || ($decimals === 0 && $fractionalPart !== null && (int) substr($fractionalPart, 0, 1) >= 5)) {
            ...
            while (++$cursor <= $integerPartLength) {
                ...
                $integerPart = substr($integerPart, 0, -$cursor).$oneUp.substr($integerPart,
                        $integerPartLength - $cursor + 1);
