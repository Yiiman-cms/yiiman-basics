...
class PhpMessageSource extends MessageSource
{
    ...
    {
        $fileId = str_replace(['/', '\\', '.', '-', '_'], '', $messageFile);
        if (is_file($messageFile)) {
            if (empty(self::$sources[$fileId])) {
                self::$sources[$fileId] = include $messageFile;
            }

            if (!is_array(self::$sources[$fileId])) {
                self::$sources[$fileId] = [];
            }

            return self::$sources[$fileId];
        }
