...
class HTML_Parser extends Node
{
    ...
    public static $_specialTags = [
        '--'      => '--',
        '[CDATA[' => ']]'
    ];
    public static $_unparsedTags = [
        'style',
        'script'
    ];
    public static $_index_attribs = [
        'href',
        'src'
    ];
    public static $_url_attribs = [
        'href' => 'href',
        'src'  => 'src'
    ];
    ...
    public static function abs_url($url, $base)
    {
        if (!self::is_url_path($url)) {
            ...
            $p = strpos($url, ':');
            if (substr($url, $p + 3, 1) === '/' && in_array(substr($url, 0, $p), [
                    'http',
                    'https'
                ])) {
    ...
    {
        $p = strpos($url, '//');
        if ($p === false || $p && !preg_match('|^[a-z]+\:$|', substr($url, 0, $p))) {
            return false;
        }
        $q = strrpos($url, '/');
        if ($p + 1 < $q) {
...
// ------------------------------------------------------------------------
// PSR-0 alias
class_exists('hQuery_HTML_Parser', false) or class_alias('YiiMan\YiiBasics\lib\hquery\\HTML_Parser',
    'hQuery_HTML_Parser', false);
