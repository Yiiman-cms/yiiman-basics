...
use HttpClientDiscovery;
use MessageFactoryDiscovery;

// ------------------------------------------------------------------------
class_exists('HTML_Parser', false) or require_once 'HTML_Parser.php';
 ...
 * @version 2.2.0
 */
class hQuery extends HTML_Parser
{
...
public static $_mockup_class;
    ...
    // ------------------------------------------------------------------------
        public $headers; // Used internally for teting
    ...
    public static function fromURL($url, $headers = null, $body = null, $options = null)
    {
        ...
        if (empty($ret)) {
            ...
            if (!empty($cch_fn)) {
                $save = self::set_cache($cch_fn, $html, [
                    'hdr'  => $hdrs,
                    'code' => $code,
                    'url'  => $url
                ]);
                ...
                switch ($rcode) {
                    ...
                    case 308:
                        ...
                        if (@$options['redirects'] > 0 && $loc = @$_rh['LOCATION']) {
                            ...
                            $loc = self::abs_url($loc, compact('host', 'port',
                                    'path') + ['scheme' => empty($options['scheme']) ? '' : $options['scheme']]);
    ...
    {
        // if ( !class_exists('YiiMan\YiiBasics\lib\hquery\HttpClientDiscovery') ) {
        //     throw new Exception('YiiMan\YiiBasics\lib\hquery\HttpClientDiscovery not found. Install a client (eg. `require php-http/curl-client`)');
        // }
        if (!isset($client)) {
        ...
        self::$last_http_result = (object) [
            'body'     => '',
            // to be set in fromHTML() - this avoids double call to __toString() on body
            'code'     => $code,
            'url'      => $url,
     ...
     * @param  \YiiMan\YiiBasics\lib\hquery\Node  $ctx   - OPTIONAL the context where to search. If omitted, $this is used.
     ...
     * @param  array|string                       $attr  - OPTIONAL attributes as string or key-value pairs.
     * @param  \YiiMan\YiiBasics\lib\hquery\Node  $ctx   - OPTIONAL the context where to search. If omitted, $this is used.
     * @return \YiiMan\YiiBasics\lib\hquery\Element collection of matched elements or NULL
     */
                    ...
                    if (isset($c['p'])) {
                        foreach ($c['p'] as $p) {
                            if (is_int($p)) {
                                ...
                                switch (key($p)) {
                                    case '<':
                                        $rc = array_slice($rc, 0, $ch, true);
                                        break;
                                    case '>':
                                        $rc = array_slice($rc, $ch, count($rc), true);
                                        break;
                                    case '-':
                                        $rc = $this->_prev($rc, $ch);
                                        break;
...
// ------------------------------------------------------------------------
// PSR-0 alias
class_exists('YiiMan\YiiBasics\lib\hquery', false);
